...
const Subject2 = ({courseData, hours}) => {
    const {selectedTheme} = useContext(ThemeContext);

    const [curriculum2, setCurriculum2] = useState();
    
  useEffect(() => {
    fetch('http://localhost:8000/courseCurriculum2')
        .then(response => {
            if (!response.ok) {
        ...
        .catch(error => {
            console.error('Error fetching curriculam 2:', error);
        });
    }, []);
  ...
  return (
    <div>
        ...
        <div className='w-full h-[350px] flex'>
            ...
            <div className='w-[640px] h-full flex flex-col'>
                ...
                <div className='flex flex-col pl-3 pt-7 w-[400px]'>
                    <p className='font-bold text-4xl mb-5'> {courseData[1]?.courseName}</p>
        ...
        <p className='font-medium text-lg text-[#464646] mt-1 pl-5'>duration: {hours[0]} Hours</p>
